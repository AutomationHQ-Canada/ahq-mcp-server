...
Follow this guide to connect Claude Code to TestBots.ai. You get **138 MCP tools** (script
...
All 138 tool descriptions are sent to the model on **every** message — MCP has no way to load them
on demand. That is a fixed **~16,000 tokens** per request, and 138 similarly-named tools for the
...
isn't recognised falls back to all 138 rather than to none — a typo costs you the saving, never the
...
```
/mcp        →  testbots-mcp-server: connected, 138 tools
...
2. Run `/mcp` → `testbots-mcp-server` should show **connected** with 138 tools.
...
| `/mcp` shows `testbots-mcp-server` **failed** | Usually `uv` missing from PATH (`uv --version` to check; restart the terminal/session after installing it). Verify the server itself with: `uv run --project <plugin folder> python -c "from src.mcp_server import TOOLS; print(len(TOOLS))"` → must print `138`. |
