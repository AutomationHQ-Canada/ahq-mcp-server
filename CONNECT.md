...
`core` is **58 tools, about half the payload** — the everyday loop (discover, author, run, report,
...
| Spec | What you get |
|---|---|
| `?profile=core` | The everyday loop — 58 tools |
...
| *(omitted)* | All 138 — the default, unchanged |
...
A misspelled profile falls back to all 138 rather than to none — you lose the reduction, never the
tools.
...
do so — an orchestrator choosing from 58 is better than from 138, but a dozen is better again.
