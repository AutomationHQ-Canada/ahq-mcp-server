...
4. If there are failures:
   - Screenshots are already in that report as `screenshotUrl` on each iteration result —
     there is no separate screenshots call. If they are missing, the run was executed
     without capture; re-run with `screenshotOnError` (or `screenshotAfterEachStep`).
